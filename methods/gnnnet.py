...
class GnnNet(MetaTemplate):
  ...
  def cuda(self):
    return super(GnnNet, self).cuda()

  def _dev(self):
    return next(self.parameters()).device

  def set_forward(self, x, is_feature=False):
    x = x.to(self._dev(), non_blocking=(self._dev().type == 'cuda'))
  ...
  def forward_gnn(self, zs):
    dev = zs[0].device
    cls_idx = torch.arange(self.n_way, device=dev).repeat_interleave(self.n_support)  # (n_way*n_support,)
    support_oh = F.one_hot(cls_idx, num_classes=self.n_way).float()                   # (n_way*n_support, n_way)
    support_oh = support_oh.view(self.n_way, self.n_support, self.n_way)             # (n_way, n_support, n_way)
    pad_query = torch.zeros(self.n_way, 1, self.n_way, device=dev)                  
    support_label = torch.cat([support_oh, pad_query], dim=1).view(1, -1, self.n_way)

    nodes = torch.cat([torch.cat([z, support_label], dim=2) for z in zs], dim=0)
    scores = self.gnn(nodes)

    scores = scores.view(self.n_query, self.n_way, self.n_support + 1, self.n_way)[:, :, -1]
    scores = scores.permute(1, 0, 2).contiguous().view(-1, self.n_way)
    return scores

  def set_forward_loss(self, x):
    y_query = torch.arange(self.n_way, device=self._dev()).repeat_interleave(self.n_query)
...
class GnnNetLRP(GnnNet):
  ...
  def _get_gnnfeature_relevance(self, gnn_nodes):
    ...
    gnn_logits = torch.log(
      LRPutil.LOGIT_BETA * (scores_sf + LRPutil.EPSILON) /
      ((1.0 + LRPutil.EPSILON) - scores_sf)
    )
  ...
  def forward_gnn(self, zs):
    dev = zs[0].device
    cls_idx = torch.arange(self.n_way, device=dev).repeat_interleave(self.n_support)
    support_oh = F.one_hot(cls_idx, num_classes=self.n_way).float().view(self.n_way, self.n_support, self.n_way)
    pad_query = torch.zeros(self.n_way, 1, self.n_way, device=dev)
    support_label = torch.cat([support_oh, pad_query], dim=1).view(1, -1, self.n_way)
    ...
    if getattr(self, 'lrptraining', False):
        ...
        scores_lrp = scores_lrp.view(self.n_query, self.n_way, self.n_support + 1, self.n_way)[:, :, -1] \
                              .permute(1, 0, 2).contiguous().view(-1, self.n_way)
        scores = scores.view(self.n_query, self.n_way, self.n_support + 1, self.n_way)[:, :, -1] \
                        .permute(1, 0, 2).contiguous().view(-1, self.n_way)
  ...
  def set_forward_loss(self, x):
    ...
    if isinstance(scores, tuple) and len(scores) == 2:
        ...
        loss = loss1 + (loss2 if self.n_support == 5 else 0.5 * loss2)
    else:
        loss = self.loss_fn(scores, y_query)
    return scores, loss